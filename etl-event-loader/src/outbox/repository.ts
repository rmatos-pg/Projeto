...
export function insertOutbox(aggregateId: string, eventType: string, payload: string): void {
  ...
  db.prepare(
    `INSERT INTO loader_outbox (id, aggregate_id, event_type, payload, status, retry_count, created_at)
     VALUES (?, ?, ?, ?, 'PENDING', 0, ?)`
...
export function findPendingOutbox(): { id: string; event_type: string; retry_count: number }[] {
  return db
    .prepare(
      `SELECT id, event_type, retry_count FROM loader_outbox WHERE status = 'PENDING' ORDER BY created_at ASC`
    )
...
export function markPublished(id: string, retryCount: number): void {
  db.prepare(
    `UPDATE loader_outbox SET status = 'PUBLISHED', retry_count = ? WHERE id = ?`
  ).run(retryCount, id);
}

export function markFailed(id: string): void {
  db.prepare(`UPDATE loader_outbox SET status = 'FAILED' WHERE id = ?`).run(id);
}
